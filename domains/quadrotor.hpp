...
BenchmarkData quadrotorBenchmark(const FileMap &params) {
	...
	// set the start & goal states
        quadrotorPtr->addStartState(quadrotor->getFullStateFromGeometricComponent(start));
	...
	quadrotor->setRobotMesh("../models/" + agentMesh);
	quadrotor->setEnvironmentMesh("../models/" + environmentMesh);
	...
	data.decomposition = quadrotor->allocDecomposition();
        // quadrotorPtr->getSpaceInformation()->printState(quadrotorPtr->getProblemDefinition()->getStartState(0), std::cout);
	return data;
}
