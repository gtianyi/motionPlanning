...
public:
    KinematicSpatialGoal(const ompl::base::SpaceInformationPtr& si,
...
class KinematicCarOptimizationObjective
        : public ompl::base::OptimizationObjective {
public:
    KinematicCarOptimizationObjective(const ompl::base::SpaceInformationPtr& si,
            double maximumVelocity,
            double goalRadius)
            : OptimizationObjective(si),
              maximumVelocity(maximumVelocity),
              goalRadius(goalRadius) {
        setCostToGoHeuristic(boost::bind(
                &KinematicCarOptimizationObjective::costToGoHeuristic,
                this,
                _1,
                _2));
    }

    ompl::base::Cost costToGoHeuristic(const ompl::base::State* a,
            const ompl::base::Goal* b) const {
        double dist =
                motionDistance(a, b->as<ompl::base::GoalState>()->getState());
    ...
    ompl::base::Cost motionCost(const ompl::base::State* s1,
            const ompl::base::State* s2) const {
        throw new ompl::Exception("KinematicCarOptimizationObjective::"
                                  "motionCost not implemented");
        return ompl::base::Cost(0);
    }

    double motionDistance(const ompl::base::State* s1,
            const ompl::base::State* s2) const {
        const ompl::base::SE2StateSpace::StateType* se21 =
                s1->as<ompl::base::SE2StateSpace::StateType>();
        const ompl::base::SE2StateSpace::StateType* se22 =
                s2->as<ompl::base::SE2StateSpace::StateType>();
...
public:
    DynamicSpatialGoal(const ompl::base::SpaceInformationPtr& si,
            const ompl::base::State* state)
            : ompl::base::GoalState(si) {
        setState(state);
        se2State = state_->as<ompl::base::CompoundStateSpace::StateType>()
...
class DynamicCarOptimizationObjective
        : public ompl::base::OptimizationObjective {
public:
    DynamicCarOptimizationObjective(const ompl::base::SpaceInformationPtr& si,
            double maximumVelocity,
            double goalRadius)
            : OptimizationObjective(si),
              maximumVelocity(maximumVelocity),
        ...
        setCostToGoHeuristic(
                boost::bind(&DynamicCarOptimizationObjective::costToGoHeuristic,
                        this,
                        _1,
                        _2));
    }
    ...
    ompl::base::Cost motionCost(const ompl::base::State* s1,
            const ompl::base::State* s2) const {
        throw new ompl::Exception(
                "DynamicCarOptimizationObjective::motionCost not implemented");
        return ompl::base::Cost(0);
    }
    ...
    double motionDistance(const ompl::base::State* s1,
            const ompl::base::State* s2) const {
        const ompl::base::SE2StateSpace::StateType* se21 =
                s1->as<ompl::base::CompoundStateSpace::StateType>()
                        ->as<ompl::base::SE2StateSpace::StateType>(0);
        const ompl::base::SE2StateSpace::StateType* se22 =
                s2->as<ompl::base::CompoundStateSpace::StateType>()
                        ->as<ompl::base::SE2StateSpace::StateType>(0);
        double dx = se21->getX() - se22->getX();
        double dy = se21->getY() - se22->getY();
...
BenchmarkData carBenchmark(const FileMap& params) {
    ...
    ompl::app::SE2RigidBodyPlanning* abstract =
    ...
    ompl::base::ScopedState<ompl::base::SE2StateSpace> start(
            car->getGeometricComponentStateSpace());
    ...
    if (params.stringVal("Domain").compare("DynamicCar") == 0) {
        auto myGoal = new DynamicSpatialGoal(car->getSpaceInformation(),
        ...
        auto myGoal = new KinematicSpatialGoal(car->getSpaceInformation(),
                car->getFullStateFromGeometricComponent(goal).get());
        myGoal->setThreshold(goalRadius);
        auto goalPtr = ompl::base::GoalPtr(myGoal);
        carPtr->setGoal(goalPtr);
    ...
    car->setRobotMesh("../models/" + agentMesh);
    car->setEnvironmentMesh("../models/" + environmentMesh);

    abstract->setRobotMesh("../models/" + agentMesh);
    abstract->setEnvironmentMesh("../models/" + environmentMesh);

    if (params.exists("MinControlDuration")) {
        carPtr->getSpaceInformation()->setMinMaxControlDuration(
                params.integerVal("MinControlDuration"),
                params.integerVal("MaxControlDuration"));
    }
    if (params.exists("PropagationStepSize")) {
        carPtr->getSpaceInformation()->setPropagationStepSize(
                params.doubleVal("PropagationStepSize"));
    }

    if (!carPtr->getSpaceInformation()
                    ->getStatePropagator()
                    ->canSteer()) // if it can steer, leave it alone!
        carPtr->getSpaceInformation()->setDirectedControlSamplerAllocator(
                directedControlSamplerAllocator);

    carPtr->setup();
    abstract->setup();

    globalParameters.abstractBounds = abstract->getStateSpace()
                                              ->as<ompl::base::SE2StateSpace>()
                                              ->getBounds();
    ...
    globalParameters.copyVectorToAbstractState = [](
            ompl::base::State* s, const std::vector<double>& values) {
    ...
    globalParameters.copyAbstractStateToVector = [](
            std::vector<double>& values, const ompl::base::State* s) {
    ...
    if (params.stringVal("Domain").compare("DynamicCar") == 0) {
        globalParameters.optimizationObjective =
                ompl::base::OptimizationObjectivePtr(
                        new DynamicCarOptimizationObjective(
                                carPtr->getSpaceInformation(),
                                maxVel,
                                goalRadius));
        // carPtr->getProblemDefinition()->setOptimizationObjective(ompl::base::OptimizationObjectivePtr(new
        // DynamicCarOptimizationObjective(carPtr->getSpaceInformation(),
        // maxVel, goalRadius)));
    } else if (params.stringVal("Domain").compare("KinematicCar") == 0) {
        globalParameters.optimizationObjective =
                ompl::base::OptimizationObjectivePtr(
                        new KinematicCarOptimizationObjective(
                                carPtr->getSpaceInformation(),
                                maxVel,
                                goalRadius));
        // carPtr->getProblemDefinition()->setOptimizationObjective(ompl::base::OptimizationObjectivePtr(new
        // KinematicCarOptimizationObjective(carPtr->getSpaceInformation(),
        // maxVel, goalRadius)));
    }
