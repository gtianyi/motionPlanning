...
BenchmarkData hovercraftBenchmark(const FileMap &params) {
	...
	hovercraft->setRobotMesh("../models/" + agentMesh);
	hovercraft->setEnvironmentMesh("../models/" + environmentMesh);
	...
	data.decomposition = hovercraft->allocDecomposition();
        hovercraftPtr->getSpaceInformation()->printState(hovercraftPtr->getProblemDefinition()->getStartState(0), std::cout);
	return data;
}
