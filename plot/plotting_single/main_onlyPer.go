...
func Q_new() {
	...
	maps := map[string][]string{
		//"2D": []string{"single-wall.dae"},
		"2D": []string{"open-area.dae", "forest.dae", "single-wall.dae", "3-ladder.dae"},
		"3D": []string{"forest.dae", "fifthelement.dae"},
	}

	resultData := map[string]map[string]string{
		"open-area.dae":    map[string]string{"P-PRM": "", "KPIECE": "", "RRT": "", "BEAST": ""},
		"single-wall.dae":  map[string]string{"P-PRM": "", "KPIECE": "", "RRT": "", "BEAST": ""},
		"3-ladder.dae":     map[string]string{"P-PRM": "", "KPIECE": "", "RRT": "", "BEAST": ""},
		"forest.dae":       map[string]string{"P-PRM": "", "KPIECE": "", "RRT": "", "BEAST": ""},
		"fifthelement.dae": map[string]string{"P-PRM": "", "KPIECE": "", "RRT": "", "BEAST": ""},
	}

	latex, err := os.Create("plotper_250_300.tex")
	if err != nil {
		panic(err)
	}
	defer latex.Close()
	...
	fmt.Fprintf(latex, "\\begin{tabular}{c c | c c c c}\nmap&algorithm&DynamicCar&Hovercraft&Quadrotor&Blimp\\\\\\hline\n")
	resultData["fifthelement.dae"]["P-PRM"] += "&--&--"
	resultData["fifthelement.dae"]["KPIECE"] += "&--&--"
	resultData["fifthelement.dae"]["RRT"] += "&--&--"
	resultData["fifthelement.dae"]["BEAST"] += "&--&--"
	for domainType, domains := range domainTypes {
		...
		for _, domain := range domains {
			...
			resultData = makeCompactBoxPlotSolvePerc(domain, "CPU Time", "Solving Time", mapDateSet, resultData)
		}
		if domainType == "2D" {
			for _, mmap := range maps[domainType] {
				if mmap == "forest.dae" {
					resultData[mmap]["P-PRM"] += " (2d)"
					resultData[mmap]["KPIECE"] += " (2d)"
					resultData[mmap]["RRT"] += " (2d)"
					resultData[mmap]["BEAST"] += " (2d)"
					continue
				}
				resultData[mmap]["P-PRM"] += "&--&--"
				resultData[mmap]["KPIECE"] += "&--&--"
				resultData[mmap]["RRT"] += "&--&--"
				resultData[mmap]["BEAST"] += "&--&--"
			}
		}
	}
	mmmaps := []string{"open-area.dae", "single-wall.dae", "3-ladder.dae",
		"forest.dae", "fifthelement.dae"}
