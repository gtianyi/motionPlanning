...
func Q_new() {
	...
	resultData := map[string]map[string]string{
		"open-area.dae":   map[string]string{"P-PRM": "", "KPIECE": "", "RRT": ""},
	...
	latex, err := os.Create("plotgat.tex")
	if err != nil {
		panic(err)
	}
	defer latex.Close()
	...
	for domainType, domains := range domainTypes {
		...
		for _, domain := range domains {
			...
			resultData = makeCompactBoxPlotGAT(domain, "CPU Time", "Solving Time", mapDateSet, resultData)
		}
		if domainType == "2D" {
			for _, mmap := range maps[domainType] {
				if mmap == "forest.dae" {
