...
const (
	dataRoot   = "../experiments/data_abeastnew_2D/"
	plottype   = ".eps"
	plotWidth  = 5.
	plotHeight = 5.
)
...
func Anytime() {
	filters := map[string]map[string]string{
		...
		"SSTStar": map[string]string{"planner": "SSTStar",
			"n0": "100000",
			"xi": "0.99",
		},
		...
		"A-BEAST": map[string]string{"planner": "AnytimeBEAST_4",
			"sstpruning":  "SSTStar",
			"costpruning": "G",
			"sampler":     "BEAST",
		},
		// "B-SST": map[string]string{"planner": "AnytimeBEASTCost_4",
		// 	"sstpruning":  "SSTStar",
		// 	"costpruning": "G",
		// 	"sampler":     "BEAST",
		// },
		"A-BEASTnew": map[string]string{"planner": "AnytimeBEASTnew_4",
			"sstpruning":  "SSTStar",
			"costpruning": "G",
			"sampler":     "BEAST",
		},
	...
	fmt.Fprintf(latex, "\\documentclass[letterpaper]{article}\n\\usepackage{geometry}\n\\usepackage{graphicx}\n\\usepackage{placeins}\n\\setlength{\\pdfpagewidth}{8.5in}\n\\setlength{\\pdfpageheight}{11in}\n\\begin{document}\n")
		...
		for _, domain := range domains {
			...
			for _, mmap := range maps[domainType] {
				for _, mesh := range meshes[domain] {
					...
					fmt.Fprintf(latex, "\\includegraphics[width=2.75in]{./%s} &\n", p3)
					fmt.Fprintf(latex, "\\includegraphics[width=2.75in]{./%s} \\\\\n", p1)
					fmt.Fprintf(latex, "\\includegraphics[width=2.75in]{./%s} &\n", p4)
					fmt.Fprintf(latex, "\\includegraphics[width=2.75in]{./%s} \\\\\n", p5)
					fmt.Fprintf(latex, "\\includegraphics[width=2.75in]{./%s} &\n", p2)
					...
					fmt.Fprintf(latex, "\\caption{\\label{fig1} {%s} results in the {%s} workspace.}\n", domain, mmap)
