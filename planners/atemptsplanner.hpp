...
class AtemptsPlanner : public ompl::control::RRT {
 public:
  ...
  AtemptsPlanner(const SpaceInformationPtr &si, const FileMap &params) :
    ...
    std::string plannerName = "AtemptsPlanner_" + whichSearch;
    setName(plannerName);

    Planner::declareParam<double>("state_radius", this,
                                  &AtemptsPlanner::ignoreSetterDouble,
                                  &AtemptsPlanner::getStateRadius);
    Planner::declareParam<unsigned int>("prm_size", this,
                                        &AtemptsPlanner::ignoreSetterUnsigedInt,
                                        &AtemptsPlanner::getPRMSize);
    Planner::declareParam<unsigned int>("num_prm_edges", this,
                                        &AtemptsPlanner::ignoreSetterUnsigedInt,
                                        &AtemptsPlanner::getNumPRMEdges);

    Planner::declareParam<double>("valid_edge_distribution_alpha", this,
                                  &AtemptsPlanner::ignoreSetterDouble,
                                  &AtemptsPlanner::getValidEdgeDistributionAlpha);
    Planner::declareParam<double>("valid_edge_distribution_beta", this,
                                  &AtemptsPlanner::ignoreSetterDouble,
                                  &AtemptsPlanner::getValidEdgeDistributionBeta);
    Planner::declareParam<double>("invalid_edge_distribution_alpha", this,
                                  &AtemptsPlanner::ignoreSetterDouble,
                                  &AtemptsPlanner::
                                  getInvalidEdgeDistributionAlpha);
    Planner::declareParam<double>("invalid_edge_distribution_beta", this,
                                  &AtemptsPlanner::ignoreSetterDouble,
                                  &AtemptsPlanner::
                                  getInvalidEdgeDistributionBeta);
    ...
    Planner::declareParam<double>("sampler_initialization_time", this,
                                  &AtemptsPlanner::ignoreSetterDouble,
                                  &AtemptsPlanner::getSamplerInitializationTime);
  }
  ...
  /*brief Continue solving for some amount of time.
  ...
  virtual base::PlannerStatus solve(
    ...
    if(!newsampler_) {
      ...
      if(whichSearch.compare("D*") == 0) {
        newsampler_ = new ompl::base::BeastSampler_dstar(
            (ompl::base::SpaceInformation *)siC_,
            pdef_->getStartState(0),
            pdef_->getGoal(),
            goal_s, params);
      } else if(whichSearch.compare("Dijkstra") == 0) {
        newsampler_ = new ompl::base::BeastSampler_dijkstra(
            (ompl::base::SpaceInformation *)siC_,
            pdef_->getStartState(0),
            pdef_->getGoal(),
            goal_s, params);
      } else {
        throw ompl::Exception("Unrecognized best first search type",
                              whichSearch.c_str());
      }
    ...
    while(ptc == false) {
      ...
      /* sample a random control that attempts to go towards the random state,
         and also sample a control duration */
      unsigned int cd = controlSampler_->sampleTo(rctrl,
                                                  nmotion->control,
                                                  nmotion->state, rmotion->state);
