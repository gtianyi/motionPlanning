...
class BeastSampler_dstarNewBonus : public ompl::base::BeastSamplerBase {
  public:
    BeastSampler_dstarNewBonus(ompl::base::SpaceInformation *base, ompl::base::State *start, const ompl::base::GoalPtr &goal,
                                  base::GoalSampleableRegion *gsr, const FileMap &params) : BeastSamplerBase(base, start, goal, gsr, params), addedGoalEdge(false) {}

    ~BeastSampler_dstarNewBonus() {}
        ...
        if(targetEdge != NULL) { //only will fail the first time through
            ...
            if(targetSuccess) {
                ...
                if(targetEdge->interior) {
                    updateEdgeEffort(targetEdge, getInteriorEdgeEffortNew(targetEdge));
                } else {
            ...
            if(k_old < k_new) {
                ...
                for(auto e : reverseEdges[u.id]) {
                    if(e.second->interior) {
                        updateEdgeEffort(e.second, getInteriorEdgeEffortNew(e.second), false);
                    }
                    else {
