...
class BeastSampler_dstarSampleTest : public ompl::base::BeastSamplerBase {
  public:
    BeastSampler_dstarSampleTest(ompl::base::SpaceInformation *base, ompl::base::State *start, const ompl::base::GoalPtr &goal,
                                      base::GoalSampleableRegion *gsr, const FileMap &params) : BeastSamplerBase(base, start, goal, gsr, params), addedGoalEdge(false) {}

    ~BeastSampler_dstarSampleTest() {}
  ...
  private:
    void doPropagationTestForEdge(edge *e){
        int n = sample_num;
                while(n > 0){
                    n--;
                    ompl::base::State *from;
                    ompl::base::State *to;
                    
                    vertexState_end =  abstraction->getState(e->endID);
                    ompl::base::ScopedState<>  fullState_end =
                            globalParameters.globalAppBaseControl->
                            getFullStateFromGeometricComponent(
                                vertexState_end);
                    fullStateSampler->
                            sampleUniformNear(to,
                                              fullState_end.get(),
                                              stateRadius);

                   
                    vertexState_start =  abstraction->getState(e->startID);
                    ompl::base::ScopedState<>  fullState_start =
                            globalParameters.globalAppBaseControl->
                            getFullStateFromGeometricComponent(
                                vertexState_start);
                    fullStateSampler->
                            sampleUniformNear(from,
                                              fullState_start.get(),
                                              stateRadius);
